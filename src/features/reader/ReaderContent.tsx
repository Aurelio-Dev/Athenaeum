...
import { analyzePdfPageVisibleContent } from "./pdfVisibleContent";
import {
...
export function ReaderContent({
  ...
  const applyNativeFullscreenVisualState = useCallback(
    (fullscreen: boolean) => {
      ...
      if (fullscreen) {
        ...
        onNativeFullscreenVisualStateChange(true);
        return;
      }

      isNativeFullscreenRef.current = false;
      setIsNativeFullscreen(false);
      onNativeFullscreenVisualStateChange(false);
    },
    [onNativeFullscreenVisualStateChange],
  );
  ...
  const flushOnUnmountRef = useRef<(readingLocation: ReadingLocation) => void>(() => {});
  flushOnUnmountRef.current = (readingLocation) => {
  ...
  useLayoutEffect(() => {
    if (flushOnUnmountTimerRef.current !== null) {
      window.clearTimeout(flushOnUnmountTimerRef.current);
      flushOnUnmountTimerRef.current = null;
    }

    return () => {
      // Captura enquanto o DOM ainda esta montado, mas adia os efeitos por um
      // tick. O segundo setup do StrictMode cancela o falso unmount; no unmount
      // real, a captura medida segue disponivel mesmo depois de destacar o DOM.
      const readingLocation = getCurrentReadingLocationRef.current();
      const flushOnUnmount = flushOnUnmountRef.current;
      flushOnUnmountTimerRef.current = window.setTimeout(() => {
        flushOnUnmountTimerRef.current = null;
        flushOnUnmount(readingLocation);
      }, 0);
    };
  }, []);
  ...
  }, [changeZoom, changeZoomBy, isActiveForShortcuts]);
  ...
  const renderHeader = (startDragging?: (event: ReactMouseEvent<HTMLElement>) => void) => (
    <ReaderFloatingChrome
          ...
          draggable={Boolean(startDragging) && !isReaderMaximized && !effectiveNativeFullscreen}
