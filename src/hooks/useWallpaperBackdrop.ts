...
export function applyWallpaperPresentation(assetUrl: string | null, opacity: number): void {
...
async function readWallpaperPresentation(source: DatabaseHandleSource) {
  const [fileName, opacity] = await Promise.all([
    getWallpaperFile(source),
    getWallpaperOpacity(source),
  ]);

  if (!fileName) {
    return { assetUrl: null, opacity };
  }
  ...
  return { assetUrl: convertFileSrc(resolvedPath), opacity };
}
...
export function useWallpaperBackdrop(databaseSource: DatabaseHandleSource): void {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      applyWallpaperPresentation(presentation.assetUrl, presentation.opacity);
    } catch (error) {
      if (refreshId === refreshIdRef.current) {
